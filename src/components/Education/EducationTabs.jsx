...
export default function EducationTabs() {
  ...
  return (
    <div className="w-full py-14 px-4 sm:px-6 text-center">
      {/* Heading */}
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3">
        Complete Digital Growth Solutions for Educational Institutions
      </h1>

      <p className="text-gray-600 max-w-4xl mx-auto mb-10 text-sm sm:text-base">
        Whether your goal is boosting inquiries, increasing campus visits, or filling classrooms...
      </p>

      {/* Tabs */}
      <div className="flex gap-3 mb-10 overflow-x-auto justify-start sm:justify-center scrollbar-hide py-2">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActive(tab.id)}
            className={`px-5 py-2 whitespace-nowrap rounded-full border transition-all text-sm font-medium
              ${
                active === tab.id
      ...
      {/* Content Box */}
      <div className="
        max-w-6xl mx-auto bg-white shadow-xl rounded-3xl p-6 sm:p-10 
